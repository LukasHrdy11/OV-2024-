...
def pouziti_fc1():
    def function2(E, potencial, rmax, l, h):
        ...
        u_over_r = u / r
        prefactor = 1 / (4 * math.pi) ** 0.5
        return u_over_r * prefactor
    ...
    integral2, error2 = quad(lambda x: -(hbarc)**2/2*my * (splev(x, tck)) * (second_derivative(lambda x: splev(x, tck), x)  + 2/x * first_derivative(lambda x: splev(x, tck), x) ) * x**2, 0, rmax)
    ...
    plt.plot(r[1:], u_over_r[1:], color='b', linestyle='-', linewidth=1)
    plt.xlabel('r [fermi]', fontsize=18)
    plt.ylabel('Vlnová funkce [a.u.]', fontsize=18)
    plt.tight_layout()
    plt.show()
